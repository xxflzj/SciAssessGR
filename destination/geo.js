 ...
 * @LastEditTime: 2023-08-24 00:08:17
 * @FilePath: \d3\geo.js
...
const g = svg.append("g").attr("transform","translate(0,40)");
    ...
    g.append("text")
        ...
        .text("Number of scientists in related affiliated countries")
    ...
    g.selectAll("path")
        ...
        .on("mouseover", function(e) {
            ...
            if (greek_sci.has(e.toElement.__data__.id)) { 
                text = greek_sci.get(e.toElement.__data__.id) + "\n" + "Greek origin scientist(s) found in" + "\n" + e.toElement.__data__.properties.name   
            } else {
                text = "None of the scientists were found in \n" + e.toElement.__data__.properties.name
            }
            tooltip.text(text)
                .style("left", (e.pageX+10) + "px")
                .style("top", (e.pageY-10) +  "px")
                .style("visibility", "visible"); 
            // white stroke for legend text 
            var temp = Math.log(greek_sci.get(e.toElement.__data__.id)) / Math.log(10)
            if (greek_sci.has(e.toElement.__data__.id)) {
                d3.selectAll(".legend_text_" + (Math.floor(temp) + 1))
                    .attr("stroke","white")    
                    .attr("stroke-width", 1.5)                
            } else {
                d3.selectAll(".legend_text_0")
                    .attr("stroke","white")
                    .attr("stroke-width", 1.5)   
            }
